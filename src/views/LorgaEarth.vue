    ...
    <section class="fragrance-view">
        ...
        <div class="lorga-earth">
            ...
            <div class="quantity">
                ...
                &nbsp<span>{{ fragrance.buy_quantity || 0}}</span>&nbsp
                <button @click="increment(fragrance.id)">+</button>
            </div>
            <br/>
            <button class="add-to-cart" :disabled="!fragrance.buy_quantity || fragrance.buy.quantity === 0"
                 @click="addToCart(fragrance)">
                ADD TO CART</button>
        </div>
    </section>
</template>
